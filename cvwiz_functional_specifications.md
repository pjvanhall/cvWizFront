...
- **Own Profile (Default Fallback)**: 
  - If no specific `id` or `medewerkerId` is provided in the Router State, the system defaults to fetching the logged-in user's CV via their profile (`getMijzelf`).
...
- **Consultant CV Provisioning (Via Router State: `medewerkerId` & `name`)**:
  - **Triggered when:** A user clicks "Edit CV" on a consultant in the `/medewerkers` list who does *not* yet have a CV (`hasCv: false`).
...
- **Direct Consultant CV Load (Via Router State: `id` & `name`)**:
  - **Triggered when:** A user clicks "Edit CV" on a consultant in the `/medewerkers` list who *already* has a CV (`hasCv: true`).
  - Fetches and loads a specific CV directly by its ID. (Sensitive IDs are hidden from the URL and passed internally).
